...
int thermodynamics_free(
  ...
  /* LFT */
  if (pth->jp_pmf_xe_file != NULL)
    free(pth->jp_pmf_xe_file);
 ...
 * helper function
 */
int thermodynamics_indices_reco(struct recombination *preco)
{
...
/**
 * LFT : struct to conform with GSL signature
 */
struct
jp_pmf_constraints_params
{
    double b;
    double f2;
    double D1;
...
/**
 * LFT : function that conforms with the GSL signature for the constraint function for the nonlinear solver
 */
static int
jp_pmf_constraints_f(const gsl_vector *x, void *p, gsl_vector *f)
// x = { f1, f3, D3 }
{
  struct jp_pmf_constraints_params *params = (struct jp_pmf_constraints_params *)p;
  double f1 = gsl_vector_get(x, 0);
  double f3 = gsl_vector_get(x, 1);
  double D3 = gsl_vector_get(x, 2);
  // unit volume
  gsl_vector_set(f, 0,
                 f1 + params->f2 + f3 - 1.0);
  // avg. baryon density
  gsl_vector_set(f, 1,
                 f1*params->D1 + params->f2*params->D2 + f3*D3 - 1.0);
  // def. of clumping parameter
  gsl_vector_set(f, 2,
                 f1*params->D1*params->D1 + params->f2*params->D2*params->D2 + f3*D3*D3 - 1.0 - params->b);
  return GSL_SUCCESS;
}

/**
 * LFT : function that conforms with the GSL signature for the constraint's function Jacobian for the nonlinear solver
 */
static int
jp_pmf_constraints_df(const gsl_vector *x, void *p, gsl_matrix *J)
{
  struct jp_pmf_constraints_params *params = (struct jp_pmf_constraints_params *)p;
  double f3 = gsl_vector_get(x, 1);
  double D3 = gsl_vector_get(x, 2);
  ...
  gsl_matrix_set(J, 1, 0, params->D1);
  gsl_matrix_set(J, 1, 1, D3);
  gsl_matrix_set(J, 1, 2, f3);
  gsl_matrix_set(J, 2, 0, params->D1*params->D1);
  gsl_matrix_set(J, 2, 1, D3*D3);
  gsl_matrix_set(J, 2, 2, 2.0*f3*D3);
  return GSL_SUCCESS;
}

/**
 * LFT : computes both the constraint function and its derivative for the nonlinear GSL solver
 */
static int
jp_pmf_constraints_fdf(const gsl_vector *x, void *p, gsl_vector *f, gsl_matrix *J)
{
  return (jp_pmf_constraints_f(x,p,f) | jp_pmf_constraints_df(x,p,J));
}

/**
 * LFT : fills the output arrays f, D with the solved three-zone model
 */
static int
jp_pmf_solve_3zone_constraints(double b, double f2, double D1, double D2,
                               double f[3], double D[3])
{
  // remove the GSL error handler so we can do the standard CLASS error handling
  gsl_set_error_handler_off();

  // initialize parameters
  struct jp_pmf_constraints_params p;
  p.b  = b;
  p.f2 = f2;
  p.D1 = D1;
  p.D2 = D2;
  ...
  FDF.f      = &jp_pmf_constraints_f;
  FDF.df     = &jp_pmf_constraints_df;
  FDF.fdf    = &jp_pmf_constraints_fdf;
  FDF.n      = 3;
  FDF.params = &p;
  // initialize state
  gsl_vector *x = gsl_vector_alloc(3);
  gsl_vector_set(x, 0, 0.33333);
  gsl_vector_set(x, 1, 0.33333);
  ...
  // check if problem has been encountered
  if (status || (iter>=9999))
  {
  ...
  f[1] = f2;
  f[2] = gsl_vector_get(s->x, 1);
  D[0] = D1;
  D[1] = D2;
  D[2] = gsl_vector_get(s->x, 2);
  // clean up
  gsl_multiroot_fdfsolver_free(s);
  gsl_vector_free(x);
  // sanity check
...
/**
 * LFT : sets the three-zone model parameters in the thermo struct
 */
static int
jp_pmf_set_params(struct thermo *pth)
{
  if (pth->jp_pmf_b < 1e-10)
  {
    pth->jp_pmf_Nzones = 1;
    pth->jp_pmf_fvect[0] = 1.0;
    pth->jp_pmf_Dvect[0] = 1.0;
  }
  else
  {
    pth->jp_pmf_Nzones = 3;
    class_call(jp_pmf_solve_3zone_constraints(pth->jp_pmf_b,
                                              pth->jp_pmf_f2,
                                              pth->jp_pmf_D1,
                                              pth->jp_pmf_D2,
                                              pth->jp_pmf_fvect,
...
int thermodynamics_recombination(
  ...
  class_call(jp_pmf_set_params(pth),pth->error_message,pth->error_message);
  ...
  {
    if (pth->jp_pmf_Nzones > 1)
    {
      class_test(_TRUE_,pth->error_message,"The three-zone model is not implemented with RECFAST.");
    }
...
int thermodynamics_recombination_with_hyrec(
  ...
  param.T0 = pba->T_cmb;
  param.obh2 = pba->Omega0_b
               *pba->h*pba->h;
  param.omh2 = (pba->Omega0_b
               +pba->Omega0_cdm+pba->Omega0_idm_dr+pba->Omega0_ncdm_tot)*pba->h*pba->h;
  ...
  for (b = 0; b < NSUBLYA; b++) twog_params.A2s_tab[b] *= param.L2s1s/L2s1s_current;
  ...
  /* LFT : loop over zones and fill the output arrays with averages */
  for (i=0; i<param.nz; i++)
  {
    xe_output[i] = Tm_output[i] = 0.0;
  }

  double *temp_xe_output = malloc(param.nz * sizeof(double));
  double *temp_Tm_output = malloc(param.nz * sizeof(double));

  for (i=0; i<pth->jp_pmf_Nzones; i++)
  {
    // modify param struct for hyrec to take changed background baryon density into account
    param.obh2 = pba->Omega0_b * pba->h * pba->h * pth->jp_pmf_Dvect[i];
    param.nH0  = 11.223846333047*param.obh2*(1.-param.Y);
    // run hyrec
    rec_build_history(&param, &rate_table, &twog_params,
                      temp_xe_output, temp_Tm_output);
    // perform the averaging
    for (j=0; j<param.nz; j++)
    {
      xe_output[j] += pth->jp_pmf_fvect[i] * pth->jp_pmf_Dvect[i] * temp_xe_output[j];
      Tm_output[j] += pth->jp_pmf_fvect[i] * pth->jp_pmf_Dvect[i] * temp_Tm_output[j];
    }
  } /* end loop over zones */
  ...
  /* LFT : give diagnostic output if user asks us to
   *
   * Note that this even works if we are not using the 3-zone model
   */
  if (pth->jp_pmf_xe_file != NULL)
  {
    double *zarr = malloc(param.nz * sizeof(double));
    for (j=0; j<param.nz; j++)
      zarr[j] = (1.0+param.zstart) * exp(-param.dlna*j) - 1.0;

    FILE *jp_pmf_xe_file = fopen(pth->jp_pmf_xe_file, "w");
    class_test(jp_pmf_xe_file == NULL,pth->error_message, "unable to open baryon_inhom_xe_file");

    fprintf(jp_pmf_xe_file, "# [0] redshift z , [1] ionization fraction xe\n");

    for (j=0; j<param.nz; j++)
      fprintf(jp_pmf_xe_file, "%.8e %.8e\n", zarr[j], xe_output[j]);

    fclose(jp_pmf_xe_file);
    free(zarr);
  }
  ...
  preco->YHe = pth->YHe;
  preco->Nnow = 3.*preco->H0*preco->H0
                *pba->Omega0_b
                *(1.-preco->YHe)/(8.*_PI_*_G_*_m_H_);
