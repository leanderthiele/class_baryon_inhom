...
int input_read_parameters(
  ...
  */
  ...
  /* LFT : whether we want to write the ionization fraction as a function of redshift to file */
  class_call(parser_read_string(pfc,"baryon_inhom_xe_file",&string1,&flag1,errmsg),errmsg,errmsg);
  if (flag1 == _TRUE_)
  {
    pth->jp_pmf_xe_file = malloc(_ARGUMENT_LENGTH_MAX_ * sizeof(char));
    strncpy(pth->jp_pmf_xe_file, string1, _ARGUMENT_LENGTH_MAX_);
    class_test(pth->jp_pmf_xe_file[_ARGUMENT_LENGTH_MAX_ - 1] != '\0',errmsg,"baryon_inhom_xe_file too long!");
    class_test(pth->recombination != hyrec,errmsg,"baryon_inhom_xe_file specified but recombination != HyRec");
  }
  else
    pth->jp_pmf_xe_file = NULL;

  /* LFT : check if three-zone model is asked for */
  class_call(parser_read_double(pfc,"baryon_inhom_f2",&(pth->jp_pmf_f2),&flag1,errmsg),errmsg,errmsg);
  class_call(parser_read_double(pfc,"baryon_inhom_D2",&(pth->jp_pmf_D2),&flag2,errmsg),errmsg,errmsg);
  class_call(parser_read_double(pfc,"baryon_inhom_b",&(pth->jp_pmf_b),&flag3,errmsg),errmsg,errmsg);
  class_call(parser_read_double(pfc,"baryon_inhom_D1",&(pth->jp_pmf_D1),&flag4,errmsg),errmsg,errmsg);
  class_test(!((flag1==flag2)&&(flag2==flag3)&&(flag3==flag4)),errmsg,"must specify all baryon_inhom options.");
  class_test(flag1 == _TRUE_ && pth->recombination != hyrec,errmsg,"if baryon_inhom options are specified, recombination must be computed with hyrec.");

  /* LFT : check if non-default 2s->1s transition rate is specified */
  class_call(parser_read_double(pfc,"L2s1s",&(pth->L2s1s),&flag1,errmsg),errmsg,errmsg);
  class_test(flag1 == _TRUE_ && pth->recombination != hyrec,errmsg,"if L2s1s is specified, recombination must be computed with hyrec.");
...
int input_default_params(
  ...
  /* LFT : default three-zone parameters */
  pth->jp_pmf_b = 0.0;
  pth->jp_pmf_f2 = 0.0;
  pth->jp_pmf_D1 = 0.0;
  pth->jp_pmf_D2 = 0.0;

  /*LFT : default 2s->1s rate (Labzowsky et al 2005) */
  pth->L2s1s = 8.2206;
