...
double rec_HPeebles_dxedlna(double xe, double nH, double H, double TM, double TR, double energy_rate, double L2s1s/*LFT*/) {
...
double rec_HRecFast_dxedlna(double xe, double nH, double H, double TM, double TR, double energy_rate, double L2s1s/*LFT*/) {
...
void interpolate_rates(double Alpha[2], double Beta[2], double *R2p2s, double TR, double TM_TR, HRATEEFF *rate_table) {
    ...
    /* LFT : we hacked the code here to make it work with relatively extreme values of the baryon inhomogeneities.
     *       The problem is that TM/TR and TR can fall outside the tabulated range.
     *       This happens for low baryon densities, so we argue that given that these low-density regions do not
     *       contribute much to the overall recombination history, all should be fine.
     */

    /* Check if TM/TR is in the range tabulated */
    /* LFT removed this check
    ...
    */
    ...
    /* LFT : if values fall outside boundary, set them to boundary
...
double rec_HMLA_dxedlna(double xe, double nH, double Hubble, double TM, double TR, double energy_rate, HRATEEFF *rate_table, double L2s1s/*LFT*/){
...
void read_twog_params(TWO_PHOTON_PARAMS *twog, double L2s1s/*LFT*/){
...
void populateTS_2photon(double Trr[2][2], double *Trv[2], double *Tvr[2], double *Tvv[3],
                        ...
                        double Alpha[2], double Beta[2], double z, double L2s1s/*LFT*/) {
...
double rec_HMLA_2photon_dxedlna(double xe, double nH, double H, double TM, double TR,
                                ...
                                unsigned iz, double z, double energy_rate, double L2s1s/*LFT*/){
   ...
   populateTS_2photon(Trr, Trv, Tvr, Tvv, sr, sv, Dtau, xe, TM, TR, nH, H, rate_table,
                      twog, fplus, fplus_Ly, Alpha, Beta, z, L2s1s/*LFT*/);
...
double xe_PostSahaH(double nH, double H, double T, HRATEEFF *rate_table, TWO_PHOTON_PARAMS *twog,
                           double zstart, double dlna, double **logfminus_hist, double *logfminus_Ly_hist[],
		    unsigned iz, double z, double *Dxe, int model, double energy_rate, double L2s1s/*LFT*/){
    ...
    if (model == 0) {         /* Peebles model */
      Ddxedlna_Dxe = (rec_HPeebles_dxedlna(xeSaha+0.01*(1.-xeSaha), nH, H, T, T, energy_rate, L2s1s)
		      - rec_HPeebles_dxedlna(xeSaha-0.01*(1.-xeSaha), nH, H, T, T, energy_rate, L2s1s))/0.02/(1.-xeSaha);
    }
    else if (model == 1)  {   /* "Recfast" model */
        Ddxedlna_Dxe = (rec_HRecFast_dxedlna(xeSaha+0.01*(1.-xeSaha), nH, H, T, T, energy_rate, L2s1s)
		      - rec_HRecFast_dxedlna(xeSaha-0.01*(1.-xeSaha), nH, H, T, T, energy_rate, L2s1s))/0.02/(1.-xeSaha);
    }
    else if (model == 2) { /* EMLA model with 2s and 2p decays only, no radiative transfer */
        Ddxedlna_Dxe = (rec_HMLA_dxedlna(xeSaha+0.01*(1.-xeSaha), nH, H, T, T, energy_rate, rate_table, L2s1s)
		      - rec_HMLA_dxedlna(xeSaha-0.01*(1.-xeSaha), nH, H, T, T, energy_rate, rate_table, L2s1s))/0.02/(1.-xeSaha);
    }
    else {    /* Default mode, with radiative transfer */
        Ddxedlna_Dxe = (rec_HMLA_2photon_dxedlna(xeSaha+0.01*(1.-xeSaha),nH, H, T, T,
                         rate_table, twog, zstart, dlna, logfminus_hist, logfminus_Ly_hist, iz, z, energy_rate, L2s1s)
                      - rec_HMLA_2photon_dxedlna(xeSaha-0.01*(1.-xeSaha), nH, H, T, T,
	                 rate_table, twog, zstart, dlna, logfminus_hist, logfminus_Ly_hist, iz, z, energy_rate, L2s1s))/0.02/(1.-xeSaha);
    }
